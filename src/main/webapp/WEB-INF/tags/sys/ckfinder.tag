...
<input id="${input}Id" style="display: none;" type="file" name="file"
	onchange="${input}SelectAction()" multiple="multiple">
...
//		var date = new Date(), year = date.getFullYear(), month = (date.getMonth()+1)>9?date.getMonth()+1:"0"+(date.getMonth()+1);
	...
	function ${input}SelectAction(){
        ...
        var files = document.getElementById("${input}Id").files;
        for(var i=0; i< files.length; i++){
            formData.append("fileArray",files[i]);   // 文件对象
        }
        //formData.append("file", document.getElementById("${input}Id").files);
        $.ajax({
            ...
            success: function (returndata) {
                ...
                var oldpath = $("#${input}").val();
                if(oldpath != "") {
                    oldpath = oldpath+"|";
                }
                $("#${input}").val(oldpath+fileUrl);
                <%--$("#${input}Preview").find('li').html(fileUrl.substring(fileUrl.lastIndexOf("/")+1));--%>
                ${input}Preview();
            },
	...
	function ${input}DelAll(){
		$("#${input}").val("");
        var file = $("#${input}Id"); file.after(file.clone()); file.remove();
		${input}Preview();
	}
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				...
				li = "<li><a href=\""+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\" download>"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"</a>";//</c:if>
