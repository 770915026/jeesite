...
<script type="text/javascript">
		...
		function batchDel(){
			...
			window.location.href="${ctx}/taier/news/batchDel?ids="+ids;
		}
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/taier/news/">新闻列表</a></li>
		<li><a href="${ctx}/taier/news/form">新闻添加</a></li>
	</ul>
		...
		<tbody>
			<c:forEach items="${page.list}" var="news">
				<tr>
					...
					<td>${fns:getDictLabel(news.isRoll, 'yes_no', '')}</td>
					<td
						style="text-overflow: ellipsis; white-space: nowrap; overflow: hidden; max-width: 150px"
						title="${news.originalLink}">${news.originalLink}</td>
					<td>${news.orderNo}</td>
					<td><a href="${ctx}/taier/news/form?id=${news.id}">修改</a> <a
						href="${ctx}/taier/news/preview?id=${news.id}">预览</a> <a
						href="${ctx}/taier/news/delete?id=${news.id}"
						onclick="return confirmx('确认要删除该新闻吗？', this.href)">删除</a> <c:if
							test="${news.delFlag=='2'}">
							<a href="${ctx}/taier/news/publishNews?id=${news.id}">发布</a>
						</c:if></td>
				</tr>
			</c:forEach>
		</tbody>
