...
<div class="accordion" id="menu-${param.parentId}">
								...
								<c:if test="${menu2.parent.id eq menu.id&&menu2.isShow eq '1'}">
									<li><a data-href=".menu3-${menu2.id}"
										...
										target="${not empty menu2.target ? menu2.target : 'mainFrame'}">
											...
											<c:if test="${menu2.href == '/taier/tlCompanyUnionInfo' }">
												<span id="companyUnionInfoNum" class="label label-info"></span>
											</c:if> <c:if test="${menu2.href == '/taier/tlApppreset' }">
												<span id="apppresetNum" class="label label-info"></span>
											</c:if>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			getUnCheckNum();
			setInterval('getUnCheckNum',5000);
		})
		function getUnCheckNum(){
			$.ajax({
				type:"POST",
				data:{},
				dataType:"json",
				url:"${ctx}/taier/tlApppreset/getUnCheckNum",
				success:function(data){
					if(data.code==200){
						var companyUnionInfoNum=data.hashMap.companyUnionInfoNum;
						var apppresetNum=data.hashMap.apppresetNum;
						$("#companyUnionInfoNum").html(companyUnionInfoNum);
						$("#apppresetNum").html(apppresetNum);
