...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/taier/news/form?id=${news.id}">新闻<shiro:hasPermission
	...
	<form:form id="inputForm" modelAttribute="news"
		action="${ctx}/taier/news/save" method="post" class="form-horizontal">
		<form:hidden path="id" />
		...
		<div class="control-group">
			<label class="control-label">是否是滚动栏新闻：</label>
			<div class="controls">
				<form:select path="isRoll" class="input-medium">
					<form:options items="${fns:getDictList('yes_no')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select>
				<%-- <form:input path="isHot" htmlEscape="false" maxlength="4" class="input-xlarge "/> --%>
