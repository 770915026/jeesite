...
<body>
	...
	<section class="qiye_cont w1200">
		...
		<div class="qiye_right" style="height: auto;" id="step2">
			...
			<table class="qiye_table table table-bordered">
				<tr>
					<td>进网型号</td>
					<td><input id="cpxh" type="text" placeholder="内网数据（此部分不可修订）"
						value="${tlApppreset.productVersion}"></td>
				</tr>
				<tr>
					<td>产品名称</td>
					<td><input id="productName" type="text"
						value="${tlApppreset.productName}"></td>
				</tr>
				<tr>
					<td>手机大版本号</td>
					<td><input id="sjbb" type="text" placeholder="厂家输入（文字、数字）"
						value="${tlApppreset.phoneVersion}"></td>
				</tr>
				<tr>
					<td>版本日期</td>
					<td><input id="bbrq" type="text" placeholder="内网数据（格式年-月-日）"
						value="${tlApppreset.versionDate}"></td>
				</tr>
				<tr>
					<td>预置应用卸载方法</td>
					<td><input id="xzfs" type="text" placeholder="厂家输入"
						value="${tlApppreset.uninstallType}"></td>
				</tr>
				<tr>
					<td>手机外观</td>
					<td>
						<div href="javascript:;" style="cursor: pointer;"
							...
							<img id="imgid" src=""
								style="width: 100%; height: 100%; position: absolute; z-index: 1000; left: 0px;" />
							<a class="dela" style="" onclick="delimg(this)"
		...
		<div class="modal" id="showAppImportDiv"
			style="background-color: transparent; width: 603px; position: fixed; margin-left: auto; margin-right: auto;">
	...
	<script>
    var unionid = "${tlCompanyUnionInfo.id}";
    var unionversion = "${tlCompanyUnionInfo.productVersion}";
    var unionproductName = "${tlCompanyUnionInfo.productName}";
    $(function(){
        addmenu();
        $("#lfywlist").show();
        $("#lflist2").show();
        ...
        var img = "${tlApppreset.img}";
        $("#imgid").attr("src",img);
        $("#sctb").val(img);

        var apps = "${tlApppreset.apps}";
        var appsary = apps.split("||");
        for(var i=0;i<appsary.length;i++) {
            var appid = appsary[i];
            $("input[value='"+appid+"']").attr("checked","true");
        }
