...
<body>
	<section>
		...
		<!--HEADER END-->
		<!--主内容-->
		<div class="taierFormula-container zcdiv">
			<!--搜索-->
			<div class="container-box">
				<h3 class="formula-tit">企业信息发布</h3>
				<br>
			...
			<div class="list-nav-box">
				<a class="list rmsj current" href="javascript:;"
					onclick="qry(1,4,0,this)">热&nbsp;点</a> <a class="list rmsj"
    ...
    function cz(){
        ...
        cc="";
        createdhl();
        $(".ppli").removeClass("cur");
        $(".ncli").removeClass("cur");
        $(".ccli").removeClass("cur");
    }
    ...
    function qry1(currpage) {
        ...
        data.nc = nc;
        data.cc = cc;
        ajax(contextPath+"/taier/jump/unionInfoListUSerQry",data,function (result) {
            ...
            if(code == 200) {
            	...
            	if(list){
            		for(var i=0;i<list.length;i++) {
                        var imgs = list[i].productImgs;
                        var img = imgs.split("||")[0];
                        $("#qiehuanlist").append('<li>'+
                    '       <img class="img" src="'+img+'+" onclick="window.location.href=\'${contextPath}/taier/jump/unionInfoDetailUSer?id='+list[i].id+'&type=5&pp='+pp+'&nc='+nc+'&cc='+cc+'\'">'+
                    '       <div class="list-text">'+
                    '       <span class="text">'+list[i].productName+'</span>'+
                    '   <div class="toumula-intro-list">'+
                    '       <a href="${contextPath}/taier/jump/unionInfoDetailUSer?id='+list[i].id+'&type=2&pp='+pp+'&nc='+nc+'&cc='+cc+'">国内证书</a>'+
                    '       <a href="${contextPath}/taier/jump/unionInfoDetailUSer?id='+list[i].id+'&type=5&pp='+pp+'&nc='+nc+'&cc='+cc+'">配置信息</a>'+
                    '       <a href="${contextPath}/taier/jump/unionInfoDetailUSer?id='+list[i].id+'&type=1&pp='+pp+'&nc='+nc+'&cc='+cc+'">产品介绍</a>'+
                    '       <a href="${contextPath}/taier/jump/unionInfoDetailUSer?id='+list[i].id+'&type=4&pp='+pp+'&nc='+nc+'&cc='+cc+'">泰尔证书</a>'+
                    '       </div>'+
                    '       </div>'+
                    '      </li>');
                    }
